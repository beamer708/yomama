...
The site is immediately public again. No code or data changes; the maintenance gate is only active when `MAINTENANCE_MODE` is `true` or `1`.

## Environment variables

| Variable              | Required when maintenance is on | Description                                      |
|-----------------------|----------------------------------|--------------------------------------------------|
| `MAINTENANCE_MODE`    | —                                | Set to `true` or `1` to enable the gate.         |
| `MAINTENANCE_PASSWORD` | Yes                            | Password for staff access (cookie set server-side). |
...
All other routes and `/api/*` are blocked when `MAINTENANCE_MODE` is on unless the request has a valid auth cookie.
